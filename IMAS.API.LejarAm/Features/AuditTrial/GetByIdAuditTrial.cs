using IMAS.API.LejarAm.Shared.Infrastructure.Persistence;
using IMAS.API.LejarAm.Shared.Models;
using MediatR;
using Microsoft.EntityFrameworkCore;

namespace IMAS.API.LejarAm.Features.AuditTrail
{
    public class GetByIdAuditTrail
    {
        public record Query : IRequest<AuditTrailDTO>
        {
            public Guid Id { get; init; }
        }

        public class Handler : IRequestHandler<Query, AuditTrailDTO>
        {
            private readonly FinancialDbContext _context;

            public Handler(FinancialDbContext context)
            {
                _context = context;
            }

            public async Task<AuditTrailDTO?> Handle(Query request, CancellationToken cancellationToken)
            {
                var entity = await _context.AuditTrial.FindAsync(new object[] { request.Id }, cancellationToken);
                return entity == null ? null : new AuditTrailDTO
                {
                    ID = entity.ID,
                    NoDoc = entity.NoDoc,
                    TarikhDoc = entity.TarikhDoc,
                    NamaPenghutang = entity.NamaPenghutang,
                    Butiran = entity.Butiran,
                    KodAkaun = entity.KodAkaun,
                    KeteranganAkaun = entity.KeteranganAkaun,
                    Debit = entity.Debit,
                    Kredit = entity.Kredit,
                    JejakAuditEntitiesID = entity.JejakAuditEntitiesID
                };
            }
        }
    }
}
